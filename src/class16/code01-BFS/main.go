...
func bfs[T any](start *graph.Node[T]) {
	if start == nil {
		return
	}
	queue := make([]*graph.Node[T], 0)
	...
	for len(queue) > 0 {
		...
		for i := range node.Nexts {
			if _, ok := set[node.Nexts[i]]; !ok {
				queue = append(queue, node.Nexts[i])
				set[node.Nexts[i]] = struct{}{}
			}
		}
	}
}
